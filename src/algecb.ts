...
import { encryptBlock, decryptBlock, additionBlock128_2, additionRevBlock2 } from './transforms';
...
export class AlgEcb {
  ...
  public encrypt(data: Uint8Array): Uint8Array {
        ...
        for (let i = 0; i < countBlocks; i++) {
            const block = paddedData.subarray(i * blockSize, (i + 1) * blockSize) as Block128;
    ...
    public decrypt(data: Uint8Array): Uint8Array {
        ...
        for (let i = 0; i < countBlocks; i++) {
            const block = decryptedData.subarray(i * blockSize, (i + 1) * blockSize) as Block128;
